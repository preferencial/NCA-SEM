...
const NCA_AUTH_CONFIG_ = {
  SESSION_KEY_PREFIX: 'NCA_SESSION_',
...
/**
 * Encontra usuario por username ou e-mail, normalizado case-insensitive.
...
function findPlaintextUser_(username) {
  try {
    ...
    for (const row of rows) {
      const rowUser = String(row[iUser] || '').toLowerCase().trim();
      const rowEmail = iEmail !== undefined ? String(row[iEmail] || '').toLowerCase().trim() : '';
      if (rowUser !== normalized && rowEmail !== normalized) continue;
      const status = iStatus !== undefined ? String(row[iStatus] || '').toLowerCase().trim() : 'active';
    ...
    try {
      try {
        try {
          ...
          const user = findPlaintextUser_(normalizedUsername);
          if (!user) {
          ...
          const session = {
            ...
            email: user.email,
            issuedAt: Date.now(),
            expiresAt: Date.now() + (NCA_AUTH_CONFIG_.SESSION_TTL_SECONDS * 1000)
          };
    
          PropertiesService.getScriptProperties().setProperty(
            NCA_AUTH_CONFIG_.SESSION_KEY_PREFIX + token,
...
function isAuthenticatedByToken(tok) {
  try {
    try {
      const token = String(tok == null ? '' : tok).trim();
      if (!token) return false;
  
      const key = NCA_AUTH_CONFIG_.SESSION_KEY_PREFIX + token;
      ...
      try {
        ...
        if (!session.userId || !session.username || !isFinite(expiresAt) || expiresAt <= Date.now()) {
...
function resolveAuthTokenFromPayload_(payloadOrToken) {
  if (typeof payloadOrToken === 'string') {
    var directToken = payloadOrToken.trim();
    return directToken || null;
  }
  
  if (typeof payloadOrToken === 'object' && payloadOrToken !== null) {
    var token = payloadOrToken._authToken || payloadOrToken.tok || '';
